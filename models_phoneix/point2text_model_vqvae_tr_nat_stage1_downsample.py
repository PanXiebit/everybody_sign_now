...
class Point2textModel(pl.LightningModule):
    def __init__(self, args, text_dict):
        ...
        self.tem_downconv1 = Downsample(512, True)
        self.tem_downconv2 = Downsample(512, True)
        ...
        self.tem_upconv1 = Upsample(512, True)
        self.tem_upconv2 = Upsample(512, True)
    ...
    def vqvae_encode(self, points, skel_len):
        points_feat = self.points_emb(points) # [bs, max_len, 512]
        points_feat = einops.rearrange(points_feat, "b t h -> b h t")

        points_feat = self.tem_downconv1(points_feat)  # [b h t/2]
        points_feat = self.tem_downconv2(points_feat)  # [b h t/4]

        points_feat = einops.rearrange(points_feat, "b h t -> b t h")

        max_len = max(skel_len)
        points_mask = self._get_mask(skel_len//4, max_len//4, points.device).unsqueeze_(1).unsqueeze_(2)

        points_feat = self.enc_vit(points_feat, points_mask) # [bs, max_len, 512]
    ...
    def vqvae_decode(self, points_feat, skel_len):
        points_feat = einops.rearrange(points_feat, "b h t-> b t h")

        max_len = max(skel_len)
        points_mask = self._get_mask(skel_len//4, max_len//4, points_feat.device).unsqueeze_(1).unsqueeze_(2)
        points_feat = self.dec_vit(points_feat, points_mask)  # [b t h]
        
        points_feat = einops.rearrange(points_feat, "b t h -> b h t")
        points_feat = self.tem_upconv1(points_feat)  # [b h t/2]
        points_feat = self.tem_upconv2(points_feat)  # [b h t/4]
        points_feat = einops.rearrange(points_feat, "b h t -> b t h")

        # reconstruction loss
        rec_feat = einops.rearrange(points_feat, "b t h -> (b t) h")
        rec_feat = self.dec_linear(rec_feat)
        dec_pose = self.pose_spl(rec_feat[:, 0:512])  # [bs*max_len, h] -> [bs*max_len, 24]
        dec_rhand = self.hand_spl(rec_feat[:, 512:1024]) # [bs*max_len, h] -> [bs*max_len, 63]
        dec_lhand = self.hand_spl(rec_feat[:, 1024:2048]) # [bs*max_len, h] -> [bs*max_len, 63]
    ...
    def forward(self, batch, mode, vis_func, vis_tok_func):
        ...
        # vqvae encoder
        vq_tokens, points_feat, commitment_loss = self.vqvae_encode(points, skel_len) # [bs, t], [bs, h, t]
        self.log('{}/commitment_loss'.format(mode), commitment_loss.detach(), prog_bar=True)

        # vqvae decoder
        dec_pose, dec_rhand, dec_lhand = self.vqvae_decode(points_feat, skel_len)
        ...
        points_mask = self._get_mask(skel_len, max_len, points_feat.device).unsqueeze_(1).unsqueeze_(2)
        rec_loss = torch.abs(dec_points - ori_points)[points_mask.view(-1)]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        rec_res = self._compute_wer(dec_points, skel_len, gloss_id, gloss_len, "test")
        ori_res = self._compute_wer(ori_points, skel_len, gloss_id, gloss_len, "test")
        return rec_res, ori_res

    def validation_epoch_end(self, outputs) -> None:
        rec_err, rec_correct, rec_count = np.zeros([4]), 0, 0
        ori_err, ori_correct, ori_count = np.zeros([4]), 0, 0
        for rec_out, ori_out in outputs:
            rec_err += rec_out["wer"]
        ...
        print("Epoch={}_count={}_wer={}".format(self.current_epoch, rec_count, rec_err[0] / rec_count))
