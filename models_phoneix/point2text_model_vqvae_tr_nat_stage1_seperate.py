...
class Point2textModel(pl.LightningModule):
    def __init__(self, args, text_dict):
        ...
        # vqvae
        self.pos_emb = nn.Parameter(torch.zeros(10000, 256))
        ...
        self.enc_vit = Encoder(dim=256, depth=3, heads=8, mlp_dim=1024, dropout = 0.1)
        self.codebook = Codebook(n_codes=args.n_codes, embedding_dim=256)
        self.dec_vit = Encoder(dim=256, depth=3, heads=8, mlp_dim=1024, dropout = 0.1)
    ...
    def vqvae_encode(self, points, points_mask):
        ...
        points_feat = torch.cat([pose, rhand, lhand], dim=2)
        points_feat = einops.rearrange(points_feat, " b t n h -> b (t n) h")
        points_feat = points_feat + self.pos_emb[:points_feat.size(1), :] # TODO
        points_feat = self.enc_vit(points_feat, points_mask.unsqueeze_(1).unsqueeze_(2)) # [bs, 3*max_len, 256]

        points_feat = einops.rearrange(points_feat, "b t h -> b h t")
        vq_output = self.codebook(points_feat)
        vq_tokens, points_feat, commitment_loss = vq_output['encodings'], vq_output['embeddings'], vq_output["commitment_loss"] # [bs, 3*max_len]
        return vq_tokens, points_feat, commitment_loss

    def vqvae_decode(self, points_feat, points_mask):
        points_feat = einops.rearrange(points_feat, "b h t-> b t h")      # [bs, 3*max_len, h]
        points_feat = points_feat + self.pos_emb[:points_feat.size(1), :] # TODO
        points_feat = self.dec_vit(points_feat, points_mask)  # [b 3*max_len h]
        
        # reconstruction loss
        rec_feat = einops.rearrange(points_feat, "b (t n) h -> b t n h", n=3) # [bs, t, 3, h]
    ...
    def forward(self, batch, mode, vis_func, vis_tok_func):
        ...
        # vqvae encoder
        vq_tokens, points_feat, commitment_loss = self.vqvae_encode(points, points_mask) # [bs, 3*t], [bs, h, 3*t]
