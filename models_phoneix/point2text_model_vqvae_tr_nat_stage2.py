...
class Point2textModelStage2(pl.LightningModule):
    def __init__(self, args, text_dict):
        ...
        self.points_mask = nn.Parameter(torch.zeros(1, 512))
    ...
    def compute_seq2seq_ce(self, gloss_id, vq_tokens, dec_feat, skel_len):
        ...
        tgt_emb_inp = dec_feat.clone()
        tgt_out = vq_tokens.new(bs, max_len).fill_(self.pad_id)

        for b in range(bs):
            cur_len = skel_len[b].item()
            sample_size = self.random.randint(int(cur_len*0.5), cur_len)
            ind = self.random.choice(cur_len, size=sample_size, replace=False)
            tgt_emb_inp[b, ind, :] = self.points_mask
            tgt_out[b, ind] = vq_tokens[b, ind]
    ...
    def forward(self, batch, mode):
        ...
        ce_loss = self.compute_seq2seq_ce(gloss_id, vq_tokens, tgt_feat, skel_len)

        self.log('{}_ce_loss'.format(mode), ce_loss.detach(), prog_bar=True)
        # total loss
        loss = commitment_loss + ce_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        skel_len = skel_len // 4
        pred_seq, _, _, out_seq_len = self.decoder.decode(pred_logits, skel_len)
    ...
    def generate(self, batch, batch_idx, total_seq_len=300, temperature = 1., default_batch_size = 1, ):
        ...
        tgt_emb_inp = self.points_mask.unsqueeze(0).repeat(bs, max_len, 1) # [bs, max_len, emb_dim]
        tgt_emb_inp = self.pos_emb(tgt_emb_inp)
        tgt_mask = self._get_mask(tgt_len, max_len, tgt_emb_inp.device).unsqueeze_(1).unsqueeze_(2)
