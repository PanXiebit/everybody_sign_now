...
class PoseSentDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        anchor = [8]
        pose_all, pose_all_no_mask = self._get_x_y_and_normalize(keypoints, anchor) # [t, 3v]
        # print("pose_all, pose_all_no_mask: ", pose_all.shape, pose_all_no_mask.shape)
        pose = pose_all[:, :, :25]
        face = pose_all[:, :, 25:25+70]
        rhand = pose_all[:, :, 25+70:25+70+21]
        lhand = pose_all[:, :, 25+70+21:25+70+21+21]

        pose_no_mask = pose_all_no_mask[:, :, :25]
        face_no_mask = pose_all_no_mask[:, :, 25:25+70]
        rhand_no_mask = pose_all_no_mask[:, :, 25+70:25+70+21]
        lhand_no_mask = pose_all_no_mask[:, :, 25+70+21:25+70+21+21]
