...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        if layer_past is None:
            if self.label_cond:
                self.mask[:, :, :32*32+1, :32*32+1] = 1
            else:
                self.mask[:, :, :32*32, :32*32] = 1
...
class CGPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_class, n_layer=12, n_head=8, n_embd=256,
    ...
    def forward(self, idx, label, embeddings=None, targets=None):
        ...
        if self.label_cond:
            cls_embed = self.cls_emb(label).unsqueeze(1)
    ...
    def forward_with_past(self, idx, label, embeddings=None, targets=None, past=None, past_length=None):
        ...
        if past is None and self.label_cond:
            cls_embed = self.cls_emb(label).unsqueeze(1)
            x = torch.cat([cls_embed, x], dim=1)
