...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        if self.perceptual_weight > 0:
            inputs_2d = inputs.permute(0, 2, 1, 3, 4).contiguous().flatten(0,1)
            recons_2d = reconstructions.permute(0, 2, 1, 3, 4).contiguous().flatten(0,1)
            p_loss = self.perceptual_loss(inputs_2d, recons_2d)
...
if __name__ == "__main__":
    vd_loss = VQLPIPSWithDiscriminator(disc_start=0, 
    ...
    recons = torch.randn(2, 3, 256, 256)
