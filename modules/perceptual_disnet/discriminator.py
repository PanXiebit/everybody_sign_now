...
class NLayerDiscriminator3d(nn.Module):
    ...
    def __init__(self, input_nc=3, ndf=64, n_layers=3, use_actnorm=False):
        ...
        kw = 4
        padw = 1
        sequence = [nn.Conv3d(input_nc, ndf, kernel_size=kw, stride=(2,2,2), padding=padw), nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
...
if __name__ == "__main__":
    ...
    y = torch.randn(2, 3, 32, 128, 128)

    model = NLayerDiscriminator3d(input_nc=3, ndf=64, n_layers=3, use_actnorm=False)
    print(model)
    logits_fake = model(x)
