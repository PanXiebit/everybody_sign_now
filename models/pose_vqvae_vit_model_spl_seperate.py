...
class PoseVitVQVAE(pl.LightningModule):
    def __init__(self, args):
        ...
        self.pose_emb = nn.Linear(96, 512)
        self.hand_emb = nn.Linear(42, 512)
        ...
        self.pose_spl = SPL(input_size=512, hidden_layers=5, hidden_units=512, joint_size=2, reuse=False, sparse=False, SKELETON="sign_pose")        
        self.hand_spl = SPL(input_size=512, hidden_layers=5, hidden_units=512, joint_size=2, reuse=False, sparse=False, SKELETON="sign_hand")        
    ...
    def encode(self, pose, rhand, lhand):
        """pose: [bs, 2, t, 8]
           ...
           lhand: [bs, 2, t, 21]
        """
        pose = einops.rearrange(pose, "b c t v -> b t n (v c)", n=1)
        pose = self.pose_emb(pose) # [bs, t, 1, h]
        rhand = einops.rearrange(rhand, "b c t v -> b t n (v c)", n=1)
        rhand = self.hand_emb(rhand) # [bs, t, 1, h]
        lhand = einops.rearrange(lhand, "b c t v -> b t n (v c)", n=1)
        lhand = self.hand_emb(lhand) # [bs, t, 1, h]

        x = torch.cat([pose, rhand, lhand], dim=-2)  # [bs, t, 3, h]
    ...
    def decode(self, x):
        ...
        x = einops.rearrange(x, "b t (c v) -> b c t v", c=2, v=48)
        return x # [bs, 2, t, 48]
    ...
    def forward(self, batch, mode):
        ...
        dec_rhand = dec_points[..., 8:29]
        dec_lhand = dec_points[..., 29:]
