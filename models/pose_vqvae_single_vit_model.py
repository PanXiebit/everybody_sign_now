...
class PoseSingleVQVAE(pl.LightningModule):
    ...
    def decode(self, feat):
        feat = self.post_vq_conv(feat).squeeze(2)
    ...
    def visualization(self, mode, name, pose, face, rhand, lhand):
        ...
        bs, c, t, v = pose.size()
        pose = pose.permute(0, 3, 1, 2).contiguous().squeeze(-1)[:8] # [bs, c, t, v]-> [bs, v, c, t] -> [8, v, c]
...
if __name__ == "__main__":
    ...
    # print(out.shape)
    x = torch.randn(2, 64, 19)

    # m1 = SamePadConv2d(64, 64, 4, (1,2))
    # x = m1(x)
    # print("x: ", x.shape)
    m0 = nn.ConvTranspose1d(64, 64, kernel_size=5, stride=1)
    m1 = SamePadConvTranspose1d(64, 64, kernel_size=4, stride=2)
    m2 = SamePadConvTranspose1d(64, 64, kernel_size=4, stride=2)
    m3 = nn.ConvTranspose1d(64, 64, kernel_size=4, stride=1) # (input - 1) * stride + output_padding - 2*padding + kernel
    x = m0(x)
    print("out: ", x.shape)
    x = m1(x)
    print("out: ", x.shape)
